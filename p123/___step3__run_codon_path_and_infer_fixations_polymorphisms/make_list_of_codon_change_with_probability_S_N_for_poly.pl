open (IN1,"list_of_codon_change_with_probability_Dsim_poly.txt");
@str1 = <IN1>;

foreach $_ (@str1) {
    if ($_ =~ /(\d+)\t(\w\w\w)\t(\w\w\w)\t(\d+)\t(\d.*)\n/) {
        $codon_pos = $1;
        $anc_codon = $2;
        $der_codon = $3;
        $freq = $4;
        ...
        open (OUT1, ">>list_of_codon_change_with_probability_S_N_Dsim_poly.txt");
        print (OUT1 "$codon_pos\t$anc_codon\t$der_codon\t$anc_AA\t$der_AA\t$freq\t$prob\t$type1\t$type2\n");
        close (OUT1);
    }
}

open (IN1,"list_of_codon_change_with_probability_Dmel_poly.txt");
@str1 = <IN1>;
...
foreach $_ (@str1) {
    if ($_ =~ /(\d+)\t(\w\w\w)\t(\w\w\w)\t(\d+)\t(\d.*)\n/) {
        ...
        open (OUT1, ">>list_of_codon_change_with_probability_S_N_Dmel_poly.txt");
        print (OUT1 "$codon_pos\t$anc_codon\t$der_codon\t$anc_AA\t$der_AA\t$freq\t$prob\t$type1\t$type2\n");
        close (OUT1);
    }
}
