for ($loop=1;$loop<=5;$loop++) {
    ...
    if ($loop == 1) {$lineage = 'Dmel';}
    if ($loop == 2) {$lineage = 'Dsim';}
    if ($loop == 3) {$lineage = 'Dyak';}
    if ($loop == 4) {$lineage = 'Dere';}
    if ($loop == 5) {$lineage = 'ye-ms';}
    
    open (IN1,"list_of_codon_change_with_probability_$lineage\_lineage_for_NG_dNdS_calculation.txt");
    @str1 = <IN1>;
    ...
    foreach $_ (@str1) {
        if ($_ =~ /(\d+)\t(\w\w\w)\t(\w\w\w)\t(\d.*)\n/) {
            ...
            open (OUT1, ">>list_of_codon_change_with_probability_S_N_$lineage\_lineage_for_NG_dNdS_calculation.txt");
