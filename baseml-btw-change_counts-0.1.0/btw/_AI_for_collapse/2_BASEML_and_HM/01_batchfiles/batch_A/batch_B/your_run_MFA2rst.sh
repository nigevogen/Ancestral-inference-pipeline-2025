...
do
	...
	rm $b
	case "$CDSopt" in
		0) mv ../04_output/tmp_result/2_alncds_cc.MFA/0.bin_int_dat_list ../04_output/tmp_result/0.bin_int_dat_list;;
	...
	if [ ${subopt[0]} -eq 1 ]; then
			 ...
			 perl check_lnL.pl > X
              ...
              do
                if [ "$line" = "1" ]
			     ...
			     mv -f rst ../../../04_output/tmp_result/5_alncds_rst_bmlo/model1_pos1_rst
			     mv -f mlf ../../../04_output/tmp_result/mlf_model1_pos1_bin"$i"
                fi;
                if [ "$line" = "0" ]
                 then
                 rm rst
			 ...
			 perl check_lnL.pl > X
              ...
              do
                if [ "$line" = "1" ]
                 ...
                 echo "Higher lnL value than previous"
                 #cp highestL highestL_"$j"
			     mv -f rst ../../../04_output/tmp_result/5_alncds_rst_bmlo/model1_pos2_rst
			     mv -f mlf ../../../04_output/tmp_result/mlf_model1_pos2_bin"$i"
                fi;
                if [ "$line" = "0" ]
                 then
                 rm rst
			 ...
			 perl check_lnL.pl > X
              ...
              do
                if [ "$line" = "1" ]
                 ...
                 echo "Higher lnL value than previous"
                 #cp highestL highestL_"$j"
			     mv -f rst ../../../04_output/tmp_result/5_alncds_rst_bmlo/model1_pos3_rst
			     mv -f mlf ../../../04_output/tmp_result/mlf_model1_pos3_bin"$i"
                fi;
                if [ "$line" = "0" ]
                 then
                 rm rst
		...
		if [ $CDSopt -eq 0 ]; then		# intron
			...
			rm lnf
			rm rates
			mv rst ../../../04_output/tmp_result/5_alncds_rst_bmlo/model1_rst
			rm rst1
			rm rub
			# BASEML end #
			cd ../../../00_bin
